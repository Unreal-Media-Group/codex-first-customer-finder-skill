...
def render_markdown(report: dict[str, Any], *, base_dir: Path | None = None) -> str:
    ...
    lines = [f"# {md(campaign['campaign_name'])}", "", "Phase 1 local dry-run report. No creative was generated and no outreach was sent.", "", "## Campaign scope", ""]
...
def render_html(report: dict[str, Any], *, base_dir: Path | None = None) -> str:
    ...
    for result in report["results"]:
        ...
        result_cards.append(f'<article><header><div><p class="eyebrow">{esc(result["duplicate_decision"]["status"])}</p><h2>{esc(result["company_name"])}</h2><code>{esc(result["identity"]["canonical_domain"])}</code></div><strong>{esc(result["score_snapshot"]["total"])}/100</strong></header><p>{esc(result["reason_for_qualification"])}</p><p><b>Uncertainty:</b> {esc(result["important_uncertainty"])}</p><p><b>Next:</b> {esc(result["recommended_next_action"])}</p>{talent}<details><summary>Evidence, score, and rejection audit</summary><ul>{signals}</ul><ul>{dimensions}</ul><ul>{rejection or "<li>No rejection reasons</li>"}</ul></details></article>')
