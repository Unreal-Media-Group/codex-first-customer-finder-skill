...
# Finalized Phase 5 activation version. Phase 6A and the Phase 6 dossier runtime
# opt into later additive versions explicitly.
SCHEMA_VERSION = 2
...
_PHASE6_INTEGRITY_COLUMNS = {
    "prospect_approval_events": {"event_snapshot", "content_hash", "byte_length", "protection_hash"},
    "enrichment_runs": {"approval_event_id", "request_fingerprint", "state", "brief_id"},
    "enrichment_sources": {"source_snapshot", "content_hash", "byte_length"},
    "campaign_brief_versions": {"brief_snapshot", "content_hash", "byte_length"},
    "brief_evidence_links": {"link_snapshot", "content_hash", "byte_length"},
    "brief_integrity_manifests": {"manifest_snapshot", "content_hash", "byte_length"},
    "brief_review_events": {"event_snapshot", "content_hash", "byte_length"},
}

_PHASE6_REQUIRED_OBJECTS = {
    "prospect_approval_root_idx",
    "brief_review_root_idx",
    "prospect_approval_events_no_update",
    "prospect_approval_events_no_delete",
    "brief_review_events_no_update",
    "brief_review_events_no_delete",
    "enrichment_sources_no_update",
    "enrichment_sources_no_delete",
    "campaign_brief_versions_no_update",
    "campaign_brief_versions_no_delete",
    "brief_evidence_links_no_update",
    "brief_evidence_links_no_delete",
    "brief_integrity_manifests_no_update",
    "brief_integrity_manifests_no_delete",
}

_PHASE6_DOSSIER_TABLES = (
    """CREATE TABLE IF NOT EXISTS dossier_searches (
        ...
        history_byte_length INTEGER NOT NULL,
        evaluation_snapshot TEXT NOT NULL,
        evaluation_hash TEXT NOT NULL,
        evaluation_byte_length INTEGER NOT NULL,
        created_at TEXT NOT NULL,
        UNIQUE (search_id, business_unit),
        UNIQUE (business_unit, idempotency_key)
    )""",
    """CREATE TABLE IF NOT EXISTS dossier_results (
        ...
        account_identity_version TEXT NOT NULL CHECK (account_identity_version = 'account-v1'),
        ...
        decision_byte_length INTEGER NOT NULL,
        selected INTEGER NOT NULL CHECK (selected IN (0,1)),
        created_at TEXT NOT NULL,
        UNIQUE (result_record_id, business_unit),
        UNIQUE (search_id, result_id),
        FOREIGN KEY (search_id, business_unit)
            REFERENCES dossier_searches(search_id, business_unit)
    )""",
    """CREATE TABLE IF NOT EXISTS dossier_approval_events (
        approval_event_id TEXT PRIMARY KEY,
        ...
        history_hash TEXT NOT NULL,
        request_hash TEXT NOT NULL,
        source_plan_snapshot TEXT NOT NULL,
        source_plan_hash TEXT NOT NULL,
        source_plan_byte_length INTEGER NOT NULL,
        scope TEXT NOT NULL CHECK (scope = 'phase6_dossier_research'),
        decision TEXT NOT NULL CHECK (decision IN ('approved','rejected','revoked','invalidated')),
        proposer_actor TEXT NOT NULL,
        reviewer_actor TEXT NOT NULL,
        reason TEXT NOT NULL,
        ...
        content_hash TEXT NOT NULL,
        byte_length INTEGER NOT NULL,
        UNIQUE (approval_event_id, business_unit),
        FOREIGN KEY (result_record_id, business_unit)
            REFERENCES dossier_results(result_record_id, business_unit),
        FOREIGN KEY (search_id, business_unit)
            REFERENCES dossier_searches(search_id, business_unit),
        FOREIGN KEY (supersedes_id, business_unit)
            REFERENCES dossier_approval_events(approval_event_id, business_unit)
    )""",
    """CREATE UNIQUE INDEX IF NOT EXISTS dossier_approval_root_idx
        ON dossier_approval_events(result_record_id, scope)
        WHERE supersedes_id IS NULL""",
    """CREATE INDEX IF NOT EXISTS dossier_approval_target_idx
        ON dossier_approval_events(result_record_id, recorded_at)""",
    """CREATE TABLE IF NOT EXISTS dossier_runs (
        dossier_run_id TEXT PRIMARY KEY,
        ...
        audit_correlation_id TEXT NOT NULL,
        UNIQUE (dossier_run_id, business_unit),
        UNIQUE (business_unit, idempotency_key),
        FOREIGN KEY (approval_event_id, business_unit)
            REFERENCES dossier_approval_events(approval_event_id, business_unit),
        FOREIGN KEY (result_record_id, business_unit)
            REFERENCES dossier_results(result_record_id, business_unit)
    )""",
    """CREATE TABLE IF NOT EXISTS dossier_candidates (
        ...
        dossier_run_id TEXT NOT NULL UNIQUE,
        approval_event_id TEXT NOT NULL UNIQUE,
        dossier_family_id TEXT NOT NULL,
        ...
        proposer_actor TEXT NOT NULL,
        created_at TEXT NOT NULL,
        UNIQUE (candidate_version_id, business_unit),
        UNIQUE (business_unit, dossier_family_id, version),
        FOREIGN KEY (dossier_run_id, business_unit)
            REFERENCES dossier_runs(dossier_run_id, business_unit),
        FOREIGN KEY (approval_event_id, business_unit)
            REFERENCES dossier_approval_events(approval_event_id, business_unit),
        FOREIGN KEY (search_id, business_unit)
            REFERENCES dossier_searches(search_id, business_unit)
    )""",
    """CREATE TABLE IF NOT EXISTS dossier_review_events (
        review_event_id TEXT PRIMARY KEY,
        ...
        content_hash TEXT NOT NULL,
        byte_length INTEGER NOT NULL,
        UNIQUE (review_event_id, business_unit),
        UNIQUE (business_unit, idempotency_key),
        FOREIGN KEY (candidate_version_id, business_unit)
            REFERENCES dossier_candidates(candidate_version_id, business_unit)
    )""",
    """CREATE TABLE IF NOT EXISTS customer_dossier_versions (
        dossier_version_id TEXT PRIMARY KEY,
        candidate_version_id TEXT NOT NULL UNIQUE,
        ...
        UNIQUE (dossier_version_id, business_unit),
        UNIQUE (business_unit, dossier_family_id, version),
        FOREIGN KEY (candidate_version_id, business_unit)
            REFERENCES dossier_candidates(candidate_version_id, business_unit),
        FOREIGN KEY (review_event_id, business_unit)
            REFERENCES dossier_review_events(review_event_id, business_unit)
    )""",
    """CREATE TABLE IF NOT EXISTS lead_intelligence_packages (
        package_record_id TEXT PRIMARY KEY,
        package_id TEXT NOT NULL UNIQUE,
        dossier_version_id TEXT NOT NULL UNIQUE,
        ...
        byte_length INTEGER NOT NULL,
        released_at TEXT NOT NULL,
        UNIQUE (package_record_id, business_unit),
        FOREIGN KEY (dossier_version_id, business_unit)
            REFERENCES customer_dossier_versions(dossier_version_id, business_unit)
    )""",
    """CREATE TRIGGER IF NOT EXISTS dossier_runs_identity_no_update
        BEFORE UPDATE ON dossier_runs
        WHEN NEW.approval_event_id IS NOT OLD.approval_event_id
          OR NEW.idempotency_key IS NOT OLD.idempotency_key
          OR NEW.request_fingerprint IS NOT OLD.request_fingerprint
          OR NEW.business_unit IS NOT OLD.business_unit
          OR NEW.result_record_id IS NOT OLD.result_record_id
          OR NEW.result_id IS NOT OLD.result_id
          OR NEW.global_identity_id IS NOT OLD.global_identity_id
          OR NEW.account_id IS NOT OLD.account_id
          OR NEW.account_identity_version IS NOT OLD.account_identity_version
          OR NEW.canonical_domain IS NOT OLD.canonical_domain
          OR NEW.initiating_actor IS NOT OLD.initiating_actor
          OR NEW.created_at IS NOT OLD.created_at
          OR NEW.audit_correlation_id IS NOT OLD.audit_correlation_id
        BEGIN SELECT RAISE(ABORT, 'dossier run identity is immutable'); END""",
    """CREATE TRIGGER IF NOT EXISTS dossier_searches_no_update
        BEFORE UPDATE ON dossier_searches
        BEGIN SELECT RAISE(ABORT, 'dossier searches are immutable'); END""",
    """CREATE TRIGGER IF NOT EXISTS dossier_searches_no_delete
        BEFORE DELETE ON dossier_searches
        BEGIN SELECT RAISE(ABORT, 'dossier searches are immutable'); END""",
    """CREATE TRIGGER IF NOT EXISTS dossier_results_no_update
        BEFORE UPDATE ON dossier_results
        BEGIN SELECT RAISE(ABORT, 'dossier results are immutable'); END""",
    """CREATE TRIGGER IF NOT EXISTS dossier_results_no_delete
        BEFORE DELETE ON dossier_results
        BEGIN SELECT RAISE(ABORT, 'dossier results are immutable'); END""",
    """CREATE TRIGGER IF NOT EXISTS dossier_approval_events_no_update
        BEFORE UPDATE ON dossier_approval_events
        BEGIN SELECT RAISE(ABORT, 'dossier approval events are append-only'); END""",
    """CREATE TRIGGER IF NOT EXISTS dossier_approval_events_no_delete
        BEFORE DELETE ON dossier_approval_events
        BEGIN SELECT RAISE(ABORT, 'dossier approval events are append-only'); END""",
    """CREATE TRIGGER IF NOT EXISTS dossier_candidates_no_update
        BEFORE UPDATE ON dossier_candidates
        BEGIN SELECT RAISE(ABORT, 'dossier candidates are immutable'); END""",
    """CREATE TRIGGER IF NOT EXISTS dossier_candidates_no_delete
        BEFORE DELETE ON dossier_candidates
        BEGIN SELECT RAISE(ABORT, 'dossier candidates are immutable'); END""",
    """CREATE TRIGGER IF NOT EXISTS dossier_review_events_no_update
        BEFORE UPDATE ON dossier_review_events
        BEGIN SELECT RAISE(ABORT, 'dossier review events are terminal and immutable'); END""",
    """CREATE TRIGGER IF NOT EXISTS dossier_review_events_no_delete
        BEFORE DELETE ON dossier_review_events
        BEGIN SELECT RAISE(ABORT, 'dossier review events are terminal and immutable'); END""",
    """CREATE TRIGGER IF NOT EXISTS customer_dossier_versions_no_update
        BEFORE UPDATE ON customer_dossier_versions
        BEGIN SELECT RAISE(ABORT, 'released dossier versions are immutable'); END""",
    """CREATE TRIGGER IF NOT EXISTS customer_dossier_versions_no_delete
        BEFORE DELETE ON customer_dossier_versions
        BEGIN SELECT RAISE(ABORT, 'released dossier versions are immutable'); END""",
    """CREATE TRIGGER IF NOT EXISTS lead_intelligence_packages_no_update
        BEFORE UPDATE ON lead_intelligence_packages
        BEGIN SELECT RAISE(ABORT, 'lead intelligence packages are immutable'); END""",
    """CREATE TRIGGER IF NOT EXISTS lead_intelligence_packages_no_delete
        BEFORE DELETE ON lead_intelligence_packages
        BEGIN SELECT RAISE(ABORT, 'lead intelligence packages are immutable'); END""",
)

_PHASE6_DOSSIER_COLUMNS = {
    "dossier_searches": {
        "search_id", "idempotency_key", "request_fingerprint", "business_unit",
        "initiating_actor", "request_snapshot", "request_hash", "request_byte_length",
        "history_snapshot", "history_hash", "history_byte_length", "evaluation_snapshot",
        "evaluation_hash", "evaluation_byte_length", "created_at",
    },
    "dossier_results": {
        "result_record_id", "search_id", "result_id", "global_identity_id", "account_id",
        "account_identity_version", "canonical_domain", "business_unit", "candidate_snapshot",
        "candidate_hash", "candidate_byte_length", "decision_snapshot", "decision_hash",
        "decision_byte_length", "selected", "created_at",
    },
    "dossier_approval_events": {
        "approval_event_id", "result_record_id", "search_id", "result_id", "global_identity_id",
        "account_id", "account_identity_version", "canonical_domain", "business_unit", "result_hash",
        "result_byte_length", "history_hash", "request_hash", "source_plan_snapshot",
        "source_plan_hash", "source_plan_byte_length", "scope", "decision", "proposer_actor",
        "reviewer_actor", "reason", "effective_at", "recorded_at", "expires_at", "supersedes_id",
        "audit_correlation_id", "event_snapshot", "content_hash", "byte_length",
    },
    "dossier_runs": {
        "dossier_run_id", "approval_event_id", "idempotency_key", "request_fingerprint",
        "business_unit", "result_record_id", "result_id", "global_identity_id", "account_id",
        "account_identity_version", "canonical_domain", "initiating_actor", "state",
        "cancel_requested", "created_at", "completed_at", "failure_class", "remediation",
        "candidate_version_id", "audit_correlation_id",
    },
    "dossier_candidates": {
        "candidate_version_id", "dossier_run_id", "approval_event_id", "dossier_family_id",
        "version", "business_unit", "result_id", "global_identity_id", "account_id",
        "account_identity_version", "canonical_domain", "search_id", "history_hash",
        "source_plan_hash", "dossier_snapshot", "content_hash", "byte_length", "proposer_actor",
        "created_at",
    },
    "dossier_review_events": {
        "review_event_id", "candidate_version_id", "idempotency_key", "request_fingerprint",
        "candidate_hash", "candidate_byte_length", "business_unit", "decision", "proposer_actor",
        "reviewer_actor", "reason", "recorded_at", "audit_correlation_id", "event_snapshot",
        "content_hash", "byte_length",
    },
    "customer_dossier_versions": {
        "dossier_version_id", "candidate_version_id", "review_event_id", "dossier_family_id",
        "version", "business_unit", "source_plan_hash", "final_snapshot", "content_hash",
        "byte_length", "released_at",
    },
    "lead_intelligence_packages": {
        "package_record_id", "package_id", "dossier_version_id", "idempotency_identity",
        "request_fingerprint", "business_unit", "source_plan_hash", "package_snapshot",
        "content_hash", "byte_length", "released_at",
    },
}

_PHASE6_DOSSIER_REQUIRED_OBJECTS = {
    "dossier_approval_root_idx",
    "dossier_approval_target_idx",
    "dossier_runs_identity_no_update",
    "dossier_searches_no_update",
    "dossier_searches_no_delete",
    "dossier_results_no_update",
    "dossier_results_no_delete",
    "dossier_approval_events_no_update",
    "dossier_approval_events_no_delete",
    "dossier_candidates_no_update",
    "dossier_candidates_no_delete",
    "dossier_review_events_no_update",
    "dossier_review_events_no_delete",
    "customer_dossier_versions_no_update",
    "customer_dossier_versions_no_delete",
    "lead_intelligence_packages_no_update",
    "lead_intelligence_packages_no_delete",
}
...
def _validate_phase6_layout(connection: sqlite3.Connection) -> None:
    ...
    if not _PHASE6_REQUIRED_OBJECTS <= objects:
        raise MissionControlError(
            500,
            "The local Phase 6A state predates the final schema-v3 integrity contract; "
            "archive it and create a new local fixture state file.",
...
def _validate_phase6_dossier_layout(connection: sqlite3.Connection) -> None:
    for table, required in _PHASE6_DOSSIER_COLUMNS.items():
        columns = {row["name"] for row in connection.execute(f"PRAGMA table_info({table})")}
        if columns != required:
            raise MissionControlError(
                500,
                "The local Phase 6 dossier state has an incomplete schema-v4 integrity contract; "
                "archive it and create a new local state file.",
            )
    objects = {
        row["name"] for row in connection.execute(
            "SELECT name FROM sqlite_master WHERE type IN ('index','trigger')"
    ...
    if not _PHASE6_DOSSIER_REQUIRED_OBJECTS <= objects:
        raise MissionControlError(
            500,
            "The local Phase 6 dossier state has an incomplete schema-v4 integrity contract; "
            "archive it and create a new local state file.",
    ...
    for statement in _PHASE6_DOSSIER_TABLES:
        match = re.match(
        ...
        if match:
            expected_sql[match.group(1)] = _normalize_schema_sql(statement)
    actual_sql = {
...
def _normalize_schema_sql(value: str) -> str:
    ...
    return re.sub(r"\bif not exists\s+", "", normalized)
...
class SqliteStore:
    ...
    def _initialize(self) -> None:
        with self.transaction() as connection:
            ...
            if current > DOSSIER_SCHEMA_VERSION:
            ...
            elif current == DOSSIER_SCHEMA_VERSION:
                _validate_phase6_layout(connection)
                _validate_phase6_dossier_layout(connection)
    ...
    def initialize_phase6_dossier(self) -> None:
        """Atomically add schema v4; only DossierService calls this boundary."""
        with self.transaction() as connection:
            row = connection.execute(
                "SELECT value FROM schema_meta WHERE key = 'schema_version'"
            ).fetchone()
            ...
            if current < PHASE6_SCHEMA_VERSION:
                raise MissionControlError(
                    500,
                    "The Phase 6 dossier service requires the complete schema-v3 prerequisite.",
                )
            _validate_phase6_layout(connection)
            if current == DOSSIER_SCHEMA_VERSION:
                _validate_phase6_dossier_layout(connection)
            ...
            for statement in _PHASE6_DOSSIER_TABLES:
                connection.execute(statement)
            self._fault("initialize_phase6_dossier")
            _validate_phase6_dossier_layout(connection)
            connection.execute(
                "UPDATE schema_meta SET value = ? WHERE key = 'schema_version'",
                (str(DOSSIER_SCHEMA_VERSION),),
            )
