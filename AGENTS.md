...
- The manifest preserves the exhausted CELSIUS/Jazwares, 4ocean/Badia, and TUUCI/Miansai routes for
  ...
  The default runtime has no executable real-proof route. The exact v4 route contains only Coolibar,
  one attempt, and no fallback; it requires the explicitly configured bounded proof service and
  durable result-bound authority. Any further proof requires a newly authorized exact target, URLs,
  source classes, and versioned route.
